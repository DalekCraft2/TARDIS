...
public class TARDISCompanionAddInventory {
    ...
    private ItemStack[] getItemStack() {
            ...
            for (Player p : plugin.getServer().getOnlinePlayers()) {
                if (i < 45) {
                    ...
                    if (puid != uuid && !comps.contains(puid.toString()) && VanishChecker.canSee(player, p)) {
                        ...
                        skull.setDisplayName(ChatColor.RESET + p.getName());
                        ArrayList<String> lore = new ArrayList<>();
                        lore.add(ChatColor.GRAY + p.getUniqueId().toString());
                        skull.setLore(lore);
        ...
        info_lore.add(ChatColor.GRAY + "Click a player head to");
        info_lore.add(ChatColor.GRAY + "add them as a companion.");
        ii.setLore(info_lore);
        info.setItemMeta(ii);
        heads[45] = info;
        ItemStack list = new ItemStack(Material.WRITABLE_BOOK, 1);
        ItemMeta ll = list.getItemMeta();
        ll.setDisplayName(ChatColor.RESET + "List companions");
        list.setItemMeta(ll);
        heads[47] = list;
        ItemStack every = new ItemStack(Material.WRITABLE_BOOK, 1);
        ItemMeta one = every.getItemMeta();
        one.setDisplayName(ChatColor.RESET + "Add all online players");
