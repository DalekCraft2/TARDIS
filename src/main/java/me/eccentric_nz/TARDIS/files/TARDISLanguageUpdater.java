...
public class TARDISLanguageUpdater {
    ...
    public void update() {
        ...
        if (i > 0) {
            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "Added " + ChatColor.AQUA + i + ChatColor.RESET + " new messages to " + lang + ".yml");
        ...
        if (j > 0) {
            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "Removed " + ChatColor.AQUA + j + ChatColor.RESET + " redundant messages from " + lang + ".yml");
        }
    }
}
