...
public class TARDISRoomMap {
        ...
        plugin.getRoomsConfig().getConfigurationSection("rooms").getKeys(false).forEach((r) -> {
            if (plugin.getRoomsConfig().getBoolean("rooms." + r + ".enabled")) {
                ...
                if (sch.exists()) {
                    ...
                    plugin.getConsole().sendMessage(plugin.getMessagePrefix() + ChatColor.RED + lower + ".tschm was not found in '" + basepath + "' and was disabled!");
    ...
    public boolean makeRoomMap(String fileStr, String s) {
        ...
        if (!f.exists()) {
            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + ChatColor.RED + "Could not find a schematic with that name!");
            return false;
        }
        // get JSON
        JsonObject obj = TARDISSchematicGZip.unzip(fileStr + ".tschm");
        if (obj == null) {
            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + ChatColor.RED + "The supplied file [" + fileStr + ".tschm] is not a TARDIS JSON schematic!");
            return false;
        } else {
            // get dimensions
                ...
                for (int row = 0; row < w; row++) {
                    ...
                    for (int col = 0; col < l; col++) {
                        ...
                        if (!(c.get("data").getAsString().contains("minecraft"))) {
                            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + ChatColor.RED + "The supplied file [" + fileStr + ".tschm] needs updating to a TARDIS v4 schematic and was disabled!");
