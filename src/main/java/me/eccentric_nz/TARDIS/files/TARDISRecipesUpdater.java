...
public class TARDISRecipesUpdater {
    ...
    public void addRecipes() {
        ...
        if (recipes_config.getString("shaped.TARDIS Materialisation Circuit.result").contains(":")) {
            ...
            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "All TARDIS and sonic screwdriver circuit recipes have changed!");
            i++;
        }
        // update for 1.13
        if (recipes_config.getString("shaped.TARDIS Locator.easy_ingredients.C").contains(":")) {
            ...
            plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "Updated recipes for Minecraft 1.13+");
            i++;
        }
        ...
        try {
            ...
            if (i > 0) {
                plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "Added " + ChatColor.AQUA + i + ChatColor.RESET + " new items to recipes.yml");
            }
            String key = recipes_config.getString("shaped.TARDIS Key.result");
            if (!key.equals(plugin.getConfig().getString("preferences.key"))) {
                plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "The TARDIS Key recipe result (recipes.yml) does not match the configured key preference (config.yml)");
            }
            String r_key_5 = recipes_config.getString("shaped.TARDIS Remote Key.easy_ingredients.K");
            if (r_key_5 != null && !key.equals(r_key_5)) {
                plugin.getConsole().sendMessage(plugin.getMessagePrefix() + "The TARDIS Key ingredient (" + r_key_5 + ") in the 'TARDIS Remote Key' recipe does not match the crafting result of the 'TARDIS Key' recipe (" + key + ") - they should be the same!");
            }
